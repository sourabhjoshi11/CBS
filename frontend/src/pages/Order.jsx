...
const OCCASIONS = [
  ...
  'Holi \u{1F3A8}',
  'Christmas \u{1F384}',
  'Valentines Day \u{1F496}',
  'Baby Shower \u{1F476}',
  'Graduation \u{1F393}',
  'Just Because \u{1F338}',
  'Other',
];
      ...
      <div className="max-w-md w-full text-center bg-white rounded-3xl p-10 shadow-2xl shadow-rose-100">
        ...
        <div className="bg-rose-50 rounded-2xl p-4 text-left mb-6 space-y-2">
          <p className="text-sm font-body text-gray-600">
            <span className="font-semibold text-rose-700">Order ID:</span> #{success.id}
          </p>
          <p className="text-sm font-body text-gray-600">
          ...
          <p className="text-sm font-body text-gray-600">
            ...
            {new Date(success.delivery_date).toLocaleDateString('en-IN', { dateStyle: 'long' })}
          </p>
        </div>
        <p className="text-gray-500 text-sm font-body mb-6">
          Saloni will contact you soon on <strong>{success.phone}</strong> to confirm your order! {'\u{1F496}'}
        </p>
        <div className="flex flex-col gap-3">
          <a
            href={`https://wa.me/+918770571002?text=Hi%20Saloni!%20I%20just%20placed%20Order%20%23${success.id}%20for%20${encodeURIComponent(success.product)}%20%F0%9F%8C%B8`}
            target="_blank"
            rel="noreferrer"
    ...
    <div className="pt-20 min-h-screen petal-bg">
      <div className="max-w-2xl mx-auto px-6 py-12">
        <div className="text-center mb-10">
          ...
          <h1 className="section-title">Place Your Order {'\u{1F338}'}</h1>
          <p className="text-gray-500 font-body mt-2">
            Fill in your details and Saloni will craft the perfect creation for you.
          </p>
        </div>

        <form onSubmit={handleSubmit} className="bg-white rounded-3xl p-8 shadow-xl shadow-rose-50 space-y-6">
          ...
          <div>
            <label className="label">{'\u{1F389}'} Occasion *</label>
            <select
            ...
            {errors.occasion && <p className="text-red-500 text-xs mt-1 font-body">{errors.occasion}</p>}
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="Custom Order">Custom Order (Describe below)</option>
            </select>
            {form.product === '' && (
              <input
            ...
            {errors.product && <p className="text-red-500 text-xs mt-1 font-body">{errors.product}</p>}
          </div>
          ...
          <div>
            ...
            <input
              type="date"
              min={today}
              className={`input-field ${errors.delivery_date ? 'border-red-300 ring-red-200' : ''}`}
              value={form.delivery_date}
              onChange={(e) => field('delivery_date', e.target.value)}
            />
            {errors.delivery_date && <p className="text-red-500 text-xs mt-1 font-body">{errors.delivery_date}</p>}
          </div>

          <div>
            <label className="label">{'\u{1F48C}'} Custom Message / Instructions</label>
            <textarea
              rows={4}
              className="input-field resize-none"
              placeholder="e.g. Please include a card saying 'Happy Birthday Riya! Love, Maa' \u{1F338}"
              value={form.message}
          ...
          {errors.submit && <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-red-600 text-sm font-body">{errors.submit}</div>}
          ...
          >
            {loading ? (
              ...
              <>
                <ShoppingBag size={18} /> Place My Order {'\u{1F338}'}
              </>
            )}
          </button>
